...
Nous alloms gerer les données manquants en utilisant la ACP (Analyse des composants principales), en generant plusieurs tableaux completés. Il va initialiser le tableau en faisant une imputation par la moyenne, on va ensuite contruire l'ACP avec le tableau completé, choisir le nombre des dimensions, les données manquantes sont imputés par ACP et le moyennes et écarts types sont mis à jour. Les étapes sont ensuite repetées jusqu'à convergence.

** tu avais fait la PCA avec des données manquantes, mais non l'imputation par ACP**
```{r pca}
nb = estim_ncpPCA(dHB,ncp.min=0,ncp.max=6)

##imputed <- imputePCA(dHB,ncp=nb$ncp, nboot=1000)
##res.pca <- PCA(imputed$completeObs)

resMI <- MIPCA(dHB,ncp=2)
...
regpca=lm(MEDV~.-INDUS-AGE,data=imputed)
summary(regpca)
