...
Au total, 120 observations sont manquantes : 20 pour chacune des variables CRIM, ZN, INDUS, CHAS, AGE, LSTAT. 
 ...
 (travail en cours) et surtout à garder? je l'aime bien mais je ne sais pas quoi dire
```{r inventDM3}
...
Nous allons ici en tester 2 : la suppression par paire et l'imputation par regression stochastique.
...
Avec cette méthode on observe une nouvelle fois que les deux variables INDUS et AGE peuvent sortir du modèle, et avec un R2 de 0.736, plus bas que celui que nous avions trouvé avec le listwise deletion.
...
La méthode par défaut est "pmm", Predictive mean matching, nous allons l'utiliser et la comparer avec rf, random forest et ACP.
...
7 - la taille de m, le nombre d'ensembles de données imputées.   
...
imp_pmm = mice(dHB, print = FALSE, m = 10, seed = 123)
fit = with(imp_pmm, lm(MEDV~1+CRIM + ZN+INDUS+CHAS+NOX+RM+AGE+DIS+RAD+TAX+PTRATIO+B+LSTAT))
#with : fill in the missing data and to analyze the data
est = pool(fit)
est

#kable(est,format="markdown",digits=3)

```


La colonne "estimate" correspond à la valeur Q. Les colonnes "ubar", "b" et "t" sont les estimateurs des variances. "dfcom" correspond aux degrés de liberté du dataset complet, df est les degrés de liberté après la correction de Barnard-Rubin. Les trois dernières colonnes sont l'augmentation relative de la variance r, la proportion de la variance dû au non réponse λ et γ la fraction d'information manquante par paramètre. 

λ est différent de 1, c'est qui pouvait arriver si toute la variation était causé par les données manquantes. 

Les montants λ, r et γ, sont des indicateurs de la gravité du problème d'absence des données. Fractions d'information manquante de 0 - 0.2 sont interprétés comme "modeste", jusqu'à 0.3 comme "moyennement grand" et 0.5 comme "élevé" (Li, Raghunathan, and Rubin 1991). Des valeurs élevées indiquent un problème difficile dans lequel les inférences statistiques dépendent fortement de la manière dont les données manquantes ont été traitées. Aucune de nos valeurs n'est supérieure à 0.1, le problème est alors modeste.

Des tests de Wald et des intervalles de confiance pour chaque element de Q sont des résultats standard de chaque procédure statistique. 
...
sumWald=summary(est, conf.int = TRUE)
kable((sumWald),format="markdown",digits=3)
#kable(ifelse((est$p.value)>0.05, cell_spec(est$p.value, "html", color = "red", bold = T), cell_spec(est$p.value, "html", color = "black", bold = T)),format="markdown",digits=3)
...
Ce résultat est cohérent par rapport aux résultats de la LM avec des données manquantes, et l'imputation stochastique.
...
L'imputation par forets aléatoires
(travail en cours)

```{r forest}
impforest = missForest(dHB,verbose = TRUE)
class(impforest)

#dHBcompl.forest=complete(impforest)
#summary(dHBcompl.forest)
#regforest1=lm(MEDV~.,data=dHBcompl.forest)
#summary(regforest1)
#regforest2=lm(MEDV~.-INDUS-AGE,data=dHBcompl.forest)
#summary(regforest2)
...
(travail en cours)
```{r pca}
nb = estim_ncpPCA(dHB,ncp.max=6)
impacp = imputePCA(dHB,ncp=6)
respca = PCA(impacp$completeObs) 
respca
```

## Selection du modèle de regression lineaire

**Stepwise selection**

Nous allons maintenant sélectionner le meilleur modèle, pour cela nous allons utiliser la méthode "stepwise selection"

```{r model,results="hide"}

##imp = mice(dHB, seed = 123, m = 10, print = FALSE)
scope = list(upper=~CRIM + ZN+CHAS+NOX+RM+DIS+RAD+TAX+PTRATIO+B+LSTAT,
              lower = ~1)
expr = expression(f1 <- lm(MEDV ~ 1),
                   f2 <- step(f1, scope = scope))
fit = with(imp_pmm, expr)
...
La fonction table compte le nombre de fois que chaque variable est présente dans les itérations. Le meilleur modèle semble être quand les 10 variables (B, CHAS, CRIM, DIS, LSTAT, NOX, PTRATIO, RAD, RM, TAX et ZN) sont présentes.
...
```{r plots_mice}
densityplot(imp_pmm, layout = c(3, 1))
