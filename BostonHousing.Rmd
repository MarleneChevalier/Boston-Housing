...
par(mfrow = c(1, 1), pch = 16, col="green")
hist(MEDV,main=NULL,xlab="valeur médiane du logement(K$)", ylab="fréquence", col="green")
...
Nous commençons par examiner les résultats d'une regression linéaire de MEDV sur les autres variables .  Le modèle est correctement ajusté (R2=0.75) mais il semble que les variables explicatives INDUS et AGE ne soient pas pertinentes pour ce modèle. (**cf. annexe 1.3**)
...
Afin de pouvoir l'utiliser, nous allons appliquer une méthode "listwise deletion" avec le risque de perte d'information et d'introduction de biais au dataset . Ce modèle écarte les variables INDUS et AGE, comme l'original, avec un R2 ajusté très proche de l'original. (**cf. annexe 1.4**)
...
La fonction aggr permet d'appréhender les données par leur proportion dans le jeux complet.
...
Le lien entre les variables maquantes et observés peut se mesurer par le tableau des "influx" et "outflux".
...
D'une façon générale, des valeurs plus grandes pour les deux valeurs sont préferées.

```{r manq}
fluxdBH=flux(dHB)[,2:3]
kable((fluxdBH),format="pandoc",digits=3)
```
...
## Imputation multiple
L'imputation correspond à l'action de convertir un échantillon non complet en un échantillon complet. Le but de l'imputation multiple est d'affecter plusieurs fois des données manquantes, d'analyser les données complétés et ensuite d'intégrer les résultats des analyses. 
...
D'habitude cette supposition est un bon départ. En plus il nous manque 22 % des données, nous sommes très loin du point problématique du 50%

2 - La forme du modèle d'imputation.  
Le choix sera orienté par l’échelle de la variable à imputer, et intègre de préférence la connaissance de la relation entre les variables
...
Selon ces résultats, il semble que 'INDUS' et 'AGE' ne soient pas des prédicteurs statistiquement pertinents pour prédire MEDV, avec un erreur de type 1 de 5%
...
La fonction table compte le nombre de fois que chaque variable est présente dans les itérations. Le meilleur modèle semble être quand les 10 variables (B, CHAS, CRIM, DIS, LSTAT, NOX, PTRATIO, RAD, RM, TAX et ZN) sont presentes. Ce modèle est cohérent avec l'exclusion d'INDUS et AGE pour le modèle. 
...
hist(CRIM,main="",xlab="taux de criminalité",ylab="fréquence", col="green")
...
par(mfrow = c(1, 2), pch = 16, col="green")
hist(NOX,main="",xlab="concentration oxyde d'azote (1/10M)",ylab="fréquence", col="green")
...
par(mfrow = c(1, 2), pch = 16, col="green")
hist(RM,main="",xlab="nombre de chambre",ylab="fréquence", col="green")
...
par(mfrow = c(1, 2), pch = 16, col="green")
hist(AGE,main="",xlab="prop. construites avant 1940",ylab="fréquence", col="green")
...
par(mfrow = c(1, 2), pch = 16, col="green")
hist(DIS,main="",xlab="distance moyenne aux centres d'emplois",ylab="fréquence", col="green")
...
par(mfrow = c(1, 2), pch = 16, col="green")
hist(RAD,main="",xlab="indice d'accessibilité des autoroutes",ylab="fréquence", col="green")
plot(RAD,MEDV,xlab="indice d'accessibilité des autoroutes",ylab="valeur médiane du logement (K$)")
abline(a=mean(MEDV),b=0, col="red")
par(mfrow = c(1, 1))
title("Valeur médiane du logement et indice d'accessibilité des autoroutes")

par(mfrow = c(1, 2), pch = 16, col="green")
hist(TAX,main="",xlab="taux d'imposition foncier (1/10K$",ylab="fréquence", col="green")
...
par(mfrow = c(1, 2), pch = 16, col="green")
hist(PTRATIO,main="",xlab="ratio élèves-enseignants",ylab="fréquence", col="green")
...
par(mfrow = c(1, 2), pch = 16, col="green")
hist(B,main="",xlab="proportion de population afro-américaine",ylab="fréquence", col="green")
plot(B,MEDV,xlab="proportion de population afro-américaine",ylab="valeur médiane du logement (K$)")
par(mfrow = c(1, 1))
title("Valeur médiane du logement et proportion de population afro-américaine")

par(mfrow = c(1, 2), pch = 16, col="green")
hist(LSTAT,main="",xlab="proportion de population précaire",ylab="fréquence", col="green")
...
par(mfrow = c(1, 2))
boxplot(CHAS,MEDV,xlab="proximité avec la rivière Charles",ylab="valeur médiane du logement (K$)", col="green")

```

### Annexe 1.2 : corrélations entre les variables au jeu de données
```{r cor}
cordBH=cor(dHB,use="complete.obs")
kable(cordBH,format="markdown",digits=2)
