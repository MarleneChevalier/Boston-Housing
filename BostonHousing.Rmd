...
L'influx d'une variable quantifie à quel point les données manquantes se connectent aux données observés des autres variables. 

L'outflux d'une variable quantifie à quel point ses données observés se connectent aux données manquantes d'autres variables. D'une façon général, des valeurs plus grandes pours les deux valeurs sont preferés. 
...
Selon ces résultats, il semblerait que les variables INDUS et AGE ne soient pas pertinents pour ce modèle. 
Nous voudrions maintenat utiliser la méthode "stepwise" pour choisir le meilleur modèle. Cependant, ce méthode ne fonctionne pas s'il y a des données manquantes. 

Afin de pouvoir l'utiliser, nous allons appliquer une méthode très répandue, "listwise deletion" avec le risque de perte d'information et d'introduction de biais au dataset, car nous allons perdre 112 observations (22% du dataset complet)
...
Ce modèle écarte les variables INDUS et AGE, comme l'original, avec un R2 ajusté très proche de l'original. 
...
MAR est plus général et plus réaliste que MCAR. La plupart des méthodes modernes de traitement des données manquantes partent de la supposition MAR.
...
1 - Décider si supposition de MAR est pausible. D'habitude cette supposition est un bon départ. En plus il nous manque 22 % des données, nous sommes très loin du point problématique du 50%
...
1 - Raw bias (RB) and percent bias (PB). Le biais brut de Q es defini comme la différence entre l'espérance de l'estimateur et le réél. Il doit être proche de zéro. 
