...
Une méthode de traitement les données manquantes peut être la suppression des lignes avec des valeurs absentes. La suppression par paire est une autre alternative à la suppression simple des lignes avec des données manquantes. Avec cette méthode, il faut calculer la moyenne, variance et covariance de toutes les données observées.
...
Nous allons ici en tester 3 : l'imputation par regression stochastique, le random forest et predictive mean matching.
...
Avec cette méthode on observe un R2 ajustée de 0.734. Proche mais plus bas que le R2 ajustée de la regression faite avec les données manquantes.
