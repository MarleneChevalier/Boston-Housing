...
Cependant, en regardant le graphique des résidus (*cf. annexe 1.5*) nous observons que le modèle n'est pas optimal, il semblerait qu'il y a une relation non linéaire avec une ou plusieurs variables. Pour la trouver, nous faisons un graphique par pairs (*cf. annexe 1.6*)  et observons que la variable LSTAT n'a pas une relation polynomiale d'ordre 2. Si nous corrigeons notre modèle en ajoutant un ordre quadratique, les résidus s'améliorent nettement. (*cf. annexe 1.7*)
...
=======
La valeur médiane du logement à Boston et ses environs est comprise entre 5K$ et 50K$. Sa distribution est croissante jusqu'a sa valeur moyenne (22.5K$) puis décroit fortement à partir de 25 K$.  

>>>>>>> aa8d8bda1d759717e2e1a0aa221fdb3858912456
...
(**cf. annexe 1.7** : résultat de la regression linéaire après selection de variable)

Les variables INDUS et AGE ne sont pas significatives dans l'explication de la valeur du logement (p-value>5% **cf. annexe 1.3**), et LSTAT a une relation quadratique avec MEDV.
...
Les méthodes d'imputation simple consistent à remplacer chaque valeur maquante par une valeur unique prédite ou simulée. 
...
Nous allons ici en tester plusieurs méthodes d'imputation multiple : l'imputation par regression stochastique, les forêts aléatoires, predictive mean matching et ACP. Nous allons appliquer ces méthodes à des modèles sans INDUS et AGE, car elles ne semblent pas pertinentes, selon les résultats des analyses précèdentes. 
...
Avec cette méthode, on observe un R2 ajusté de **0.7806** pour la régression linéaire faite avec le dataset completé avec la régression stochastique. C'est une R2 ajusté proche de celui obtenu avec la régression faite avec les données manquantes.

La description de la nouvelle structure des données se trouve sur l'**annexe 3.2**. Sur cette description nous observons que pour deux variables (CRIM et LSTAT), l'imputation propose des valeurs négatifs en sachant que sur le dataset d'origine ces deux variables sont toujours positives.

### Imputation par forêts aléatoires

Avec cette méthode, les valeurs sont imputées en faisant des tirages aléatoires à partir des distributions gaussiennes independantes, centrées en les moyennes prédites par les forêts aléatoires.  
Pour faire cette imputation, nous utilisons la fonction **mice ( avec method = "rf")**, avec m=6
...
Avec cette méthode on observe un R2 ajusté de **0.783** avec le dataset completé, presque le même obtenu avec la méthode d'imputation stochastique.  Les valeurs estimées et les résidus restent aussi très proches.
...
De même que pour les deux méthodes précédentes, le R2 ajusté est très proche du modèle avec des données manquantes (**0.7808**). Les valeurs estimées et les résidus restent aussi très proches.
...
```
Selon le graphique, le nombre de dimensions sera de 3.
...
Nous avons testé 4 méthodes d'imputation, qui nous ont permis de faire des régressions avec le jeu de données complet. Ces régressions ont des résultats très proches en terme de R2 ajusté, des estimateurs et des résidus. Nous allons faire des diagnostics pour choisir le meilleur modèle d'imputation. 
...
Les méthodes de régression stochastique et ACP imputent des valeurs plus bas que les observées pour les variables CRIM, ZN et AGE. Cela fait qu'ils existent des valeurs minimums beaucoup plus petits que ceux de la distribution original. Par contre, les méthodes pmm et forêts aléatoires, respectent mieux la distribution des variables.

Pour la variable CRIM, les valeurs observées plus grandes se trouvent uniquement sur l'imputation par forêts aléatoires. 

Pour la variable AGE, la régression stochastique et ACP imputent des valeurs plus grands que ceux observées, proches de 200 pour les imputés quand la valeur maximale des observées est autour de 100.
...
Après nos trois diagnostics il semblerait que nos modèles avec les différentes méthodes:
...
* Ne présentent pas de problèmes de convergence
* Avec l'overimputation nous voyons que les variables CRIM et ZN ne sont pas toujours bien representées. En CRIM c'est visible principalement pour les grands valeurs.

Si on compare par contre en termes de rapidité machine, la méthode APC c'est celle qui demande plus de temps, même en sachant que nous avons un dataset plutôt petit.

Les résidus sont très proches pour tous les modèles (annexe 4.2), donc pour notre petit dataset, nous aurions obtenu les mêmes résultats pour la régression linéaire sans l'imputation. 
...
### Annexe 1.5 : Graphique des résidus du modèle
```{r res}
plot(model, 1)
```

### Annexe 1.6 : pairs plot
```{r pairs}
pairs(~ CRIM + ZN + CHAS + NOX + RM + DIS + MEDV, data = dHB)
pairs(~ RAD + TAX + PTRATIO + B + LSTAT + MEDV, data = dHB)
```

### Annexe 1.7 : regression linéaire multiple avec ordre 2
...
## Annexe 4.2 Comparaison des residuals 

```{r residualspmm}
par(mfrow = c(2, 2))
plot(model, 1, main="original")
plot(regrf,1,main="Forêts aléatoires")
plot(regregsto,1,"Régression stochastique")
plot(regpmm, 1, main ="pmm")
```
