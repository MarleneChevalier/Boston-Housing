...
Voici une deuxième representation des données manquantes, avec matrixplot. Elle permet d'identifier des dépendences, des patterns des valeurs extrêmes et des données manquantes. Les données observés sont dans une échelle de gris et les manquantes en rouge. Les petits valeurs sont plus claires, et les valeurs plus grands en gris foncé.

```{r inventDM3}
matrixplot(dHB,sortby="MEDV")
```

Nous pouvons observer qu'il n'y pas des outliers dans le dataframe et les valeurs manquantes semblent bien se repartir dans l'ensemble. 
...
## Lien variables manquantes / variables observées (** à garder cette partie? trop repetitive peut-être**)
...
**Créer des dataframes complets**

Une méthode de traitement des données manquantes peut être la suppression des lignes avec des valeurs absentes, pour avoir un dataset complet. La suppression par paire est une alternative à la suppression simple des lignes avec des données manquantes. Avec cette méthode, il faut calculer la moyenne, variance et covariance de toutes les données observées.
...
Nous observons que cette méthode, comme prévu, va supprimer les lignes avec des données manquantes, donc le dataframe d'analyse a uniquement 394 lignes. Selon les résultats du p-value, INDUS et AGE peuvent sortir du modèle, elles ne sont pas representatives. 
...
5 - L'ordre d'imputation des variables (influe sur la convergence de l'algorithme). Par défaut, algorithme MICE impute les données incomplètes du dataset de gauche à droite. L'ordre est à changer si nous avons des soucis de convergence des algorithmes. 
...
Nous allons ici en tester 3 méthodes d'imputation simple : l'imputation par regression stochastique, les forêts aléatoires et le predictive mean matching. Nous allons appliquer les trois méthodes à des modèles sans INDUS et AGE, car elles ne semblent pas pertinentes, selon les résultats des analyses précèdents. (Annexe 1.4)
...
Avec cette méthode on observe un R2 ajustée de 0.734. Proche mais plus bas que le R2 ajustée de la regression faite avec les données manquantes. Les valeus estimées et les residus restent aussi très proches.
...
Pareil que pour les deux méthodes précedents, le R2 ajusté est très proche du modèle avec des données manquantes. Les valeus estimées et les residus restent aussi très proches.


**Diagnostic**

Un premier diagnostic à réaliser est de vérifier que la distribution des données imputées est similaire à celle des données d'origine. 
...
Pour la variable CRIM, aucune méthode ne semble pas bien arriver à réproduire la même distribution. Par contre pour INDUS, AGE et LSTAT, les trois méthodes semblent bien arriver à le faire. Pour CHAS et ZN, les forêts aléatoires sont les plus proches de la vrai distribution.

Un deuxième diagnostic consiste en vérifier la convergence des 3 algorithmes. 

```{r diagnostics2, warning=FALSE}
...
Voici les graphiques des variations de la moyenne et l'écart type, avec les trois méthodes, pour chaque itération et chaque donnée imputée. Pour la convergence, les différentes lignes doivent se mélanger librement sans une tendance définitive. C'est bien le cas pour nos graphiques, donc, nous n'avons pas des problèmes de convergence. 

Les trois méthodes semblent bien converger sans montrer aucune tendance.

En dernier, nous allons vérifier l'ajustement du modèle d'imputation, en utilisant le graph d'overimputation. Dans ce cas, chaque donnée observée et supprimée et pour chaque, 100 valeurs sont predits (en utilisant la même méthode d'imputation choisie) et la moyenne et des intervalles de confiance de 90% sont calculés pour ces valeurs. 

Sur ces graphiques, la ligne y=x apparaît pour indiquer où les imputations devraient être si elles étaient parfaites, ainsi que les points et les intervalles de confiance pour chaque valeur. On espère que 90% des intervalles doivent passer par cette ligne y=x. La couleur des intervalles represente la fraction des données manquantes (entre 0 - 20% pour notre cas)

Voici le graphique pour la regression stochastique :
...
Le graphique pour les forêts aléatoires
...
Le dernier pour la predictive mean matching
...
Comme CHAS n'est pas une variable continue, elle prends uniquement les valeurs 0 et 1, elle n'apparaît pas dans les graphiques. 

On observe que pour les variables INDUS, AGE et LSTAT la plupart des intervalles de confiance contiennent bien la ligne diagonale pour les trois méthodes. Cependant ce n'est pas les cas pour les variables CRIM et ZN

**Conclusion**

Les résultats des trois modes d'imputation nous conduisent vers des modèles qui sont très similaires, avec les mêmes estimateurs, mêmes résidus et le même R2 ajustée.Cependant la méthode des forêts aléatoires semble mieux respecter la distribution des variables. 

Nous allons maintenant faire de l'imputation multiple pour pouvoir comparer les résultats. 
...
D'abord nous allons chercher le nombre des dimensions
```{r dim_pca}
...
Selon les graphiques les intervalles touchent tous la ligne diagonal, sauf pour les plus grands valeurs de CRIM. Il semblerait que cette méthode est plus appropié pour faire de l'imputation sur notre dataset. 

```{r pca2}

plot(res.bayesMIPCA, choice = "var")

```

Ce graphique represente la projection des individus pour chaque dataset imputé, avec l'algorithme iterative de l'ACP. Nous voyons qu'il y a très peu d'incertitude sur les variables (les nuages des points sont très rapprochés)
...
Les valeurs des estimateurs sont très proches aux ceux trouvés par l'imputation simple, mais les imputations respectent mieux la distribution avec cette méthode. 
