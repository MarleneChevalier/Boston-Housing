...
Nous commençons par examiner les résultats d'une regression linéaire de MEDV sur les autres variables. Le modèle est correctement ajusté (R²ajusté=0.76) mais il semble que les variables explicatives INDUS et AGE ne soient pas pertinentes pour ce modèle. (**cf. annexe 1.3**)
...
**Le meilleur modèle sur dataset incomplet testé ici est la regression lineaire du prix médian du logement sur l'ensemble des variables explicatives, hormis INDUS et AGE et avec LSTAT².**
...
Nous allons ici tester plusieurs méthodes d'imputation multiple : l'imputation par regression stochastique, les forêts aléatoires, predictive mean matching et l'ACP. Nous allons appliquer ces méthodes à des modèles sans INDUS et AGE, car elles ne semblent pas pertinentes, selon les résultats des analyses précèdentes. 
...
Avec cette méthode, on observe un R2 ajusté de **`r regsto.adjr2`** après la régression linéaire faite avec le dataset completé. C'est une R2 ajusté proche de celui obtenu avec la régression faite avec les données manquantes.
...
Avec cette méthode on observe un R2 ajusté de **`r regrf.adjr2`** avec le dataset completé, proche de celui obtenu avec la méthode d'imputation stochastique.  Les valeurs estimées et les résidus restent aussi très proches.
...
De même que pour les deux méthodes précédentes, le R2 ajusté **`r regpmm.adjr2`**est très proche du modèle avec des données manquantes. Les valeurs estimées et les résidus restent aussi très proches.
...
Selon le graphique (cf. **annexe 3.5**), le nombre de dimensions à retenir sera de 3.
...
Nous avons testé 4 méthodes d'imputation, qui nous ont permis de faire des régressions avec le jeu de données complet. Ces régressions ont des résultats très proches en terme de R2 ajusté, d'estimateurs et de résidus. Nous allons faire des diagnostics pour choisir le meilleur modèle d'imputation. 
...
* Ne permettent pas de bien reconstruire les variables CRIM et ZN, qui ne sont pas toujours bien représentées (cf. overimputation).

Par contre, si on compare les méthodes d'imputation en terme de rapidité machine, la méthode APC est celle qui demande plus de temps, même sur un dataset plutôt petit.

Les résidus sont très proches quelques soient les modèles (cf.**annexe 4.2**). 

Donc pour notre petit dataset, nous aurions obtenu les mêmes résultats pour la régression linéaire sans l'imputation. 
