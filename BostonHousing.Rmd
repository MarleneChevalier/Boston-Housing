...
**Le meilleur modèle sur dataset incomplet testé ici est la regression lineaire du prix médian du logement sur l'ensemble des variables explicatives, hormi INDUS et AGE et avec LSTAT².**
...
Avec cette méthode, on observe un R2 ajusté de **0.7806** après la régression linéaire faite avec le dataset completé. C'est une R2 ajusté proche de celui obtenu avec la régression faite avec les données manquantes.
...
Avec cette méthode on observe un R2 ajusté de **0.783** avec le dataset completé, proche de celui obtenu avec la méthode d'imputation stochastique.  Les valeurs estimées et les résidus restent aussi très proches.
